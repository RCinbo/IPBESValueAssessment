...
combinedNCP_by_mf_df = foreach (idx = 1:32, .combine = "rbind") %do% {
  ...
  combinedNCP_by_mf = sapply(MF_cols, FUN = function(mf_colname) by(combined.data[ ,idx+2], INDICES = s3_single_MF[,mf_colname], FUN = sum, na.rm=T))
...
p#df("output/Fig_6.3_pie_12Nov.pdf", width=10, height = 10, pointsize = 18)
